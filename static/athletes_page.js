...
function createHeaderHTML(rlvtKeys) {
    ...
    for(let i = 0; i < rlvtKeys.length; i++) {
        headerHTML+='<th><button onclick="loadTable(\''+rlvtKeys[i]+'\')">'+rlvtKeys[i]+'</button></th>';
    }
...
function createAthleteHTML(athlete) {
    ...
    for(let i = 0; i < rlvtTimes.length; i++) {
        rowHTML += '<td>'+fmtTime(rlvtTimes[i])+'</td>';
    }
    return rowHTML;
}
